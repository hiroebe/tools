...
func sortImports(localPrefix, projectModulePath string, fset *token.FileSet, f *ast.File) {
	for i, d := range f.Decls {
		...
		for j, s := range d.Specs {
			if j > i && fset.Position(s.Pos()).Line > 1+fset.Position(d.Specs[j-1].End()).Line {
				...
				specs = append(specs, sortSpecs(localPrefix, projectModulePath, fset, f, d.Specs[i:j])...)
				i = j
			}
		}
		specs = append(specs, sortSpecs(localPrefix, projectModulePath, fset, f, d.Specs[i:])...)
		d.Specs = specs
...
func sortSpecs(localPrefix, projectModulePath string, fset *token.FileSet, f *ast.File, specs []ast.Spec) []ast.Spec {
	...
	sort.Sort(byImportSpec{localPrefix, projectModulePath, specs})
...
func (x byImportSpec) Less(i, j int) bool {
	...
	igroup := importGroup(x.localPrefix, x.projectModulePath, ipath)
	jgroup := importGroup(x.localPrefix, x.projectModulePath, jpath)
	if igroup != jgroup {
